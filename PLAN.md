...
- **Stack**: Next.js 15 (App Router) + TypeScript + PostgreSQL + Drizzle ORM + Redis + Tailwind CSS + shadcn/ui + Recharts
- **Deployment**: Vercel (frontend + API routes) + Supabase (PostgreSQL) + Upstash (Redis)
...
- **Stack**: React/Vite (frontend) + Hono/Fastify (API) + PostgreSQL + Redis
- **Deployment**: Cloudflare Pages (frontend) + Fly.io or Railway (API) + Neon (PostgreSQL)
...
**RepoPopIndex** is a web application that quantifies how actively GitHub repositories and Hugging Face models/datasets are being used in the real world. It goes beyond vanity metrics (stars, likes) by analyzing commit velocity, contributor diversity, issue responsiveness, dependency adoption, release cadence, social buzz, and star/like quality — producing a single composite "Popularity Index" score (0-100) with full drill-down transparency.
...
| Database | PostgreSQL (Supabase) | Relational, JSON support for raw API responses |
| ORM | Drizzle ORM | Type-safe, lightweight, SQL-like syntax |
| Cache | Redis (Upstash) | ALL caching (API responses, reports, rate limits, progress) |
...
[User Input: URL] 
    ...
    -> [Inngest Function: "analysis.run"]
        ...
        -> [Raw Metrics Store (PostgreSQL)]
        -> [Scoring Engine]
            |-- [Metric Normalizer]
            |-- [Category Score Calculator]
            |-- [Composite Score Calculator]
        -> [Report stored in PostgreSQL + cached in Redis]
...
3. **Redis-Only Caching**: Redis (Upstash) handles ALL caching: completed reports, individual API responses, rate limit counters, and analysis progress. No PostgreSQL `api_cache` table. Redis is faster and has native TTL support. Cache key format: `rpi:report:{platform}:{owner}/{repo}:{period}`.
...
import { pgTable, text, integer, real, timestamp, jsonb, uuid, pgEnum } from 'drizzle-orm/pg-core';

export const platformEnum = pgEnum('platform', ['github', 'huggingface']);
export const statusEnum = pgEnum('status', ['queued', 'collecting', 'scoring', 'complete', 'partial', 'failed']);
...
export const analyses = pgTable('analyses', {
  ...
  repo: text('repo').notNull(),             // repo name or model name
...
// Raw metrics collected from APIs
export const rawMetrics = pgTable('raw_metrics', {
  id: uuid('id').defaultRandom().primaryKey(),
  analysisId: uuid('analysis_id').references(() => analyses.id, { onDelete: 'cascade' }).notNull(),
  source: text('source').notNull(),         // 'github_graphql', 'github_rest', 'github_search', 'hackernews', etc.
  category: text('category').notNull(),     // 'G1', 'G2', ..., 'S1', etc.
  metricKey: text('metric_key').notNull(),  // 'star_count', 'commit_count', etc.
  rawValue: real('raw_value'),              // Numeric value
  rawJson: jsonb('raw_json'),              // Full API response for complex data
...
export const scores = pgTable('scores', {
  ...
  categoryScores: jsonb('category_scores').notNull(),       // { "G-Activity": 72, "G-Community": 85, ... }
  metricScores: jsonb('metric_scores').notNull(),           // { "G1.1": { raw: 5000, normalized: 0.78, weighted: 0.58 }, ... }
  excludedCategories: jsonb('excluded_categories'),         // ["G-Community"] with reasons
  starQualityFactor: real('star_quality_factor'),           // 0-1
  starQualityRecent: real('star_quality_recent'),           // 0-1 (recent sample group)
  starQualityHistorical: real('star_quality_historical'),   // 0-1 (historical sample group)
  starBurstDetected: integer('star_burst_detected').default(0), // boolean
  scoredAt: timestamp('scored_at').defaultNow().notNull(),
});
```

**Migration strategy**: Drizzle Kit generates SQL migrations to the `drizzle/` directory. Migrations run automatically via `pnpm db:migrate` in the Vercel build step (`package.json` build script: `"build": "pnpm db:migrate && next build"`). During local development, use `pnpm db:push` for rapid iteration and `pnpm db:generate` to create migration files before committing.

### 7.2 Indexes

```sql
CREATE INDEX idx_analyses_platform_owner_repo ON analyses(platform, owner, repo);
CREATE INDEX idx_analyses_status ON analyses(status);
CREATE INDEX idx_raw_metrics_analysis ON raw_metrics(analysis_id);
CREATE INDEX idx_raw_metrics_category ON raw_metrics(category);
CREATE INDEX idx_scores_analysis ON scores(analysis_id);
```

### 7.3 Redis Cache Keys

Redis is the **sole caching layer** (no PostgreSQL `api_cache` table). All cache operations go through Redis.
...
- Hero section: Title, tagline ("Measure real repository popularity, not vanity metrics")
...
- [ ] Database setup: Drizzle schema + PostgreSQL + Drizzle Kit migrations (`drizzle/` directory, `pnpm db:migrate` in build step)
- [ ] URL parser: GitHub and HF URL validation and parsing
- [ ] Rate limiter + retry utility with 202 handling
- [ ] Redis cache layer (sole caching solution — no PostgreSQL cache table)
...
- `pnpm db:migrate` runs migrations from `drizzle/` directory successfully
...
### Decision
Build RepoPopIndex as a Next.js 15 full-stack application with PostgreSQL, Redis (sole cache layer), and Inngest for background jobs, using a composite scoring methodology inspired by OpenSSF Criticality Score. Use GitHub Search API for time-filtered issue/PR counts. Use polling (not SSE) for progress updates.
...
6. **PostgreSQL `api_cache` Table for Caching**: Rejected in favor of Redis-only caching. Redis is faster, has native TTL support, and avoids the complexity of managing cache in two systems.
...
- **Neutral**: PostgreSQL is the correct choice regardless of architecture — metrics data is inherently relational
...
pnpm add drizzle-orm @neondatabase/serverless  # or postgres
