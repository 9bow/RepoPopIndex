...
| `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY` | 배포 시 | Inngest로 분석을 백그라운드 실행 |
...
| 변수 | 발급 절차 |
|---|---|
| `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `REDDIT_USER_AGENT` | https://www.reddit.com/prefs/apps → **create another app...** → script 타입 → `client id`(앱 이름 아래)와 `secret` 복사. UA는 `web:com.example.repopopindex:v1.0 (by /u/handle)` 형태 (generic UA는 차단됨). |
| `STACKEXCHANGE_KEY` (선택) | 익명도 300 req/IP/day 가능. https://stackapps.com/apps/oauth/register 에서 등록 후 **Key**(Client Id가 아님)를 복사하면 10,000 req/day로 상향. |
| `YOUTUBE_API_KEY` | https://console.cloud.google.com/ → 프로젝트 생성 → **APIs & Services → Library**에서 "YouTube Data API v3" 활성화 → **Credentials → Create Credentials → API key**. 권장: API 제한을 YouTube Data API v3로 한정. 기본 쿼터 10,000 unit/day, 분석당 ~101 unit 사용. |
